...
def ivfpq_inference(index, query_features, final_vids, features, id_index):
    """
...
def eval_recall(annotations, relevant_labels, dataset, set_names):
    recall = []
    iterations = viewitems(annotations)
    for query, gt_sets in iterations:
        query = str(query)
        # set of relevant videos
        query_gt = set(sum([gt_sets[label] for label in relevant_labels if label in gt_sets], []))
        ...
        count = 0
        for video in set_names[query]:
            if video in dataset and video in query_gt:
                count += 1
        recall.append(count / len(query_gt))
        print('Query:{}\t\tRecall={:.4f}'.format(query, count / len(query_gt)))
...
if __name__ == '__main__':
    ...
    # 每段的聚类中心点的个数（或者说码）占用的bit数，8意味256个聚类中心点（每段）
    index = faiss.IndexIVFPQ(quantizer, d, nlist, m, 8)
    index.train(np.array(features).reshape(-1, 512))
    index.add(np.array(features).reshape(-1, 512))  # add may be a bit slower as well
    ...
    for task_name in ['DSVR', 'CSVR', 'ISVR']:
        ...
        if set_names is None:
            ...
            for query_name in query_names:
                ...
                ## Stage-1
                # cluster_vids, cluster_features = hnsw_inference(faiss_index, query_features, final_vids, features, id_index)
                cluster_vids, cluster_features = ivfpq_inference(index, query_features, final_vids, features, id_index)
                print(len(cluster_vids), len(cluster_features))
                print('HNSW Inference time: %.2f'% (time.time() - start))
                ## Evaluate Recall
        ...
        recall = eval_recall(annotations=gtobj.annotations, relevant_labels=relevant_labels_mapping[task_name],
                            dataset=gtobj.dataset, set_names = set_names)

        print('{} recall is {}'.format(task_name, np.mean(recall)))
