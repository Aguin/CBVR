...
import json
import time
from knn import knn_inference,knn_init
...
from kmeans import kmeans_inference
...
if __name__ == '__main__':
    ...
    # ## Create Annoy Model
    # start = time.time()
    # tree=Annoy.knn_init(features[:,0:16])
    # print('annoy_init time: %.2f'% (time.time() - start))
    # # print(tree)
    # tree.save('/home/malei/annoy.ann')
    ...
    for task_name in ['DSVR', 'CSVR', 'ISVR']:
        ...
        for query_name in query_names:
            ...
            ## Stage-1
            cluster_vids, cluster_features = Annoy.knn_inference(tree, query_features, final_vids, features, id_index)
            print(len(cluster_vids), len(cluster_features))
            print('Annoy Inference time: %.2f'% (time.time() - start))
            ## Stage-2
            start = time.time()
            query_result = process_single_query(query_name, query_features, cluster_vids, cluster_features, vid2names)
            print('Stage-2 time: %.2f' % (time.time() - start))
