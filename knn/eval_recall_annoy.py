...
def hnsw_inference(faiss_index, query_features, final_vids, features, id_index):
    k = 1000
    _, frame_index = faiss_index.search(query_features, k)
    set_vids = set(final_vids[frame_index.reshape(-1)])
...
if __name__ == '__main__':
    ...
    start = time.time()
    tree = AnnoyIndex(16)
    tree.load('/home/malei/annoy.ann')
    print('annoy_load time: %.2f' % (time.time() - start))
    ...
    for task_name in ['DSVR', 'CSVR', 'ISVR']:
        ...
        if set_names is None:
            ...
            for query_name in query_names:
                ...
                ## Stage-1
                cluster_vids, cluster_features = knn_inference(tree, query_features, final_vids, features, id_index)
                print(len(cluster_vids), len(cluster_features))
                print('annoy Inference time: %.2f'% (time.time() - start))
                ## Evaluate Recall
