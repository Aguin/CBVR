...
def eval_recall(annotations, relevant_labels, dataset, set_names):
    ...
    for query, gt_sets in iterations:
        ...
        precision.append(count / len(set_names[query]))
        print('Query:{}\t\tRecall={:.4f}\t\tPrecision={:.4f}'.format(query, count / len(query_gt), count / len(set_names[query])))
...
if __name__ == '__main__':
    ...
    # Load HNSW
    faiss_index = faiss.IndexHNSWFlat(512, 32)
    ...
    for task_name in ['DSVR', 'CSVR', 'ISVR']:
        ...
        recall, precision = eval_recall(annotations=gtobj.annotations, relevant_labels=relevant_labels_mapping[task_name],
