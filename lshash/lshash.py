        ...
        if results is None:
            ...
            for query_name in query_names:
                ...
                for i in range(D.shape[0]):
                    dis = cdist(query_features[i].reshape(1, -1), features[I[i]], metric='cosine')
                    for j in range(D.shape[1]):
                        vid_score[final_vids[I[i, j]]] += 1 - dis[0, j]
                query_result = {vid2names[vid][0]: score for vid, score in vid_score.items()}
